Router.configure({
  ...
  waitOn: function() { 
    return [Meteor.subscribe('notifications'),Meteor.subscribe('currentWords')]
  }
});
...
Router.map(function() {
  ...
  this.route('wordPage', {
    ...
    waitOn: function() {
      return [
        Meteor.subscribe('wordPosts', this.params._id)
      ];
    },
    data: function() { return Posts.find({wordId : this.params._id}); }
  });
